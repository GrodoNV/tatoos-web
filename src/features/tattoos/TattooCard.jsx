    ...
    <div className="bg-gray-900 rounded-3xl overflow-hidden border border-gray-800 group hover:border-purple-500/50 transition-all duration-500 shadow-2xl hover:shadow-purple-500/10">
      <div className="relative aspect-[4/5] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
          <p className="text-purple-400 text-[10px] md:text-xs font-black uppercase tracking-widest mb-1">{tattoo.style}</p>
          <h3 className="text-white font-black text-xl md:text-2xl uppercase leading-none mb-2">{tattoo.title}</h3>
          <p className="text-gray-400 text-xs md:text-sm line-clamp-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
            {tattoo.description}
          </p>
        </div>
        ...
        {/* Controles de Admin */}
        {isAdmin && (
          <div className="absolute top-3 right-3 md:top-4 md:right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <button 
              onClick={(e) => { e.stopPropagation(); onEdit(tattoo); }}
              className="bg-purple-600 hover:bg-purple-500 text-white p-3 md:p-3 rounded-xl md:rounded-2xl shadow-xl backdrop-blur-md transition-all active:scale-90"
            ...
            {isActualAdmin && (
              <button 
                ...
                className="bg-red-600/20 hover:bg-red-600 text-red-500 hover:text-white p-3 md:p-3 rounded-xl md:rounded-2xl shadow-xl backdrop-blur-md border border-red-500/30 transition-all active:scale-90"
